 ...
 */
public class ExperienceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all experiences").get("/api/experiences").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new experience")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created experience").get("${new_experience_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created experience").delete("${new_experience_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Experience entity").exec(scn);
