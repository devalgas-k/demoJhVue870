 ...
 */
public class MessageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all messages").get("/api/messages").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new message")
                        .post("/api/messages")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created message").get("${new_message_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created message").delete("${new_message_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Message entity").exec(scn);
