 ...
 */
public class LocationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all locations").get("/api/locations").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new location")
                        .post("/api/locations")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created location").get("${new_location_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created location").delete("${new_location_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Location entity").exec(scn);
