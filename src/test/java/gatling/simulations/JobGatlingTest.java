 ...
 */
public class JobGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all jobs").get("/api/jobs").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created job").get("${new_job_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created job").delete("${new_job_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Job entity").exec(scn);
