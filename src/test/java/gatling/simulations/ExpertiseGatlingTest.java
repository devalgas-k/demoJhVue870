...
public class ExpertiseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all expertise").get("/api/expertise").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new expertise")
                        .post("/api/expertise")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"title\": \"SAMPLE_TEXT\"" + ", \"description\": null" + ", \"level\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_expertise_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created expertise").get("${new_expertise_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created expertise").delete("${new_expertise_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Expertise entity").exec(scn);
