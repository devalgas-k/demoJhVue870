...
describe('Location e2e test', () => {
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      cy.get(`[data-cy="streetAddress"]`).type('du fait que vide spécialiste');
      cy.get(`[data-cy="streetAddress"]`).should('have.value', 'du fait que vide spécialiste');

      cy.get(`[data-cy="postalCode"]`).type('lâche touriste lorsque');
      cy.get(`[data-cy="postalCode"]`).should('have.value', 'lâche touriste lorsque');

      cy.get(`[data-cy="city"]`).type('Le Tampon');
      cy.get(`[data-cy="city"]`).should('have.value', 'Le Tampon');

      cy.get(`[data-cy="stateProvince"]`).type('avouer à travers');
      cy.get(`[data-cy="stateProvince"]`).should('have.value', 'avouer à travers');
